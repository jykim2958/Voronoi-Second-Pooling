...
class RandomSmoothAP:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        # # Find the positives_per_query farthest positives for each query
        # s_positives = s_qz.detach().clone()
        # s_positives.masked_fill_(torch.logical_not(positives_mask), np.PINF)
        # #closest_positives_ndx = torch.argmax(s_positives, dim=1).view(-1, 1)  # Indices of closests positives for each query
        # closest_positives_ndx = torch.topk(s_positives, k=self.positives_per_query, dim=1, largest=False, sorted=True)[1]
        # closest_positives_ndx is (batch_size, positives_per_query)  with positives_per_query closest positives
        # per each batch element
        # NOTE choose random positives
        closest_positives_ndx = torch.multinomial(positives_mask.float(), self.positives_per_query)
        ...
        # Mean ranking of selected positive examples (farthest positives)
        temp = s_diff.detach() > 0
        temp = torch.logical_and(temp[:, 0], negatives_mask)        # Take the worst positive
        hard_ranking = temp.sum(dim=1)
        stats['best_positive_ranking'] = hard_ranking.float().mean(dim=0).item()
        # Recall at 1
        # stats['recall'] = {1: (hard_ranking <= 1).float().mean(dim=0).item()} # NOTE jykim just calculate R@1!
        tmp_qz = s_qz.detach().clone()
        tmp_qz.fill_diagonal_(np.NINF)
        recall_1_ndx = torch.topk(tmp_qz, k=1, dim=1, largest=True, sorted=True)[1]
        stats['recall'] = {1: torch.gather(positives_mask, 1, recall_1_ndx).float().mean().item()}
...
class TruncatedSmoothAP:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        # Recall at 1
        # stats['recall'] = {1: (hard_ranking <= 1).float().mean(dim=0).item()} # NOTE jykim just calculate R@1!
        tmp_qz = s_qz.clone().detach()
        # tmp_qz.fill_diagonal_(np.NINF)
        tmp_qz.fill_diagonal_(-np.inf)
        recall_1_ndx = torch.topk(tmp_qz, k=1, dim=1, largest=True, sorted=True)[1]
        stats['recall'] = {1: torch.gather(positives_mask, 1, recall_1_ndx).float().mean().item()}

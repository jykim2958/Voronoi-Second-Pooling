...
from models.losses.smoothap import SmoothAP, TruncatedSmoothAP, RandomSmoothAP
...
def make_losses(params: TrainingParams):
    if params.loss == 'batchhardtripletmarginloss':
    ...
    elif params.loss == 'smoothap':
        loss_fn = SmoothAP(tau1=params.tau1, similarity=params.similarity,
        ...
        loss_fn = TruncatedSmoothAP(tau1=params.tau1, similarity=params.similarity,
                                    positives_per_query=params.positives_per_query)
    elif params.loss == 'randomsmoothap':
        loss_fn = RandomSmoothAP(tau1=params.tau1, similarity=params.similarity,
                                 positives_per_query=params.positives_per_query)
    else:
...
class BatchHardTripletLossWithMasks:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        stats = {'loss': loss.item(), 'avg_embedding_norm': self.loss_fn.distance.final_avg_query_norm,
                 # 'num_non_zero_triplets': self.loss_fn.reducer.triplets_past_filter, # NOTE https://github.com/KevinMusgrave/pytorch-metric-learning/issues/556
                 'num_non_zero_triplets': len(loss[loss>0]),

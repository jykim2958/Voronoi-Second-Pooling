...
class MaxClipRandomPoints:
    def __init__(self, max_num=35000):
    ...
    def __call__(self, e):
        ...
        if n <= self.max_num:
            return e
        mask = np.random.choice(range(n), size=self.max_num, replace=False)
        e[mask] = torch.zeros_like(e[mask])
        return e


# class FarthestPointSample:
#     def __init__(self, max_num=4096):
#         self.max_num = max_num
# 
#     def __call__(self, e):
#         n = len(e)
...
#             return e
#         e = e.unsqueeze(0)
#         e, _ = sample_farthest_points(e, K=self.max_num, random_start_point=True)
#         return e.squeeze(0)
